...
class HR_Sueldos(models.Model):
    ...
    def default_get(self, fields):
        ...
        for emp in employees:
            ...
            nomina_lines.append((0, 0, {
                'empleado_id': emp.id,
                'rut': emp.work_contact_id.document_number if emp.work_contact_id else 'no',
    ...
    # Agregar este método nuevo en la clase HR_Sueldos

    def action_actualizar_nomina(self):
        """Actualiza los datos de la nómina manteniendo el estado de validación"""
        # Verificar que sea el mes actual
        current_month_year = f"{datetime.now().month:02d}{datetime.now().year}"
        if self.month_year != current_month_year:
            raise UserError(_('No se puede actualizar una nómina de un mes diferente al actual.'))
        
        # Actualizar la fecha de la nómina
        self.write({'fecha': fields.Date.today()})
        ...
        for emp in employees:
            # Calcular licencias (holiday_status_id = 2)
            licencia_dias = 0
            ausencias = self.env['hr.leave'].search([
                ('employee_id', '=', emp.id),
                ('holiday_status_id', '=', 2),  # Licencias
            ...
            for permiso in permisos:
                ...
                end_datetime = min(permiso.date_to, last_day)
                delta = end_datetime - start_datetime
                permisos_horas += delta.total_seconds() / 3600
            
            # Calcular permisos en días (holiday_status_id = 6)
            permisos_dias = 0
            permisos_d = self.env['hr.leave'].search([
                ('employee_id', '=', emp.id),
                ('holiday_status_id', '=', 6),  # Permisos dias
        ...
        # Escribir las nuevas líneas
        self.write({
            'nomina_id': nomina_lines,
            'nomina_id_base': base_lines,
            'nomina_id_bonos': bonos_lines,
        })
        ...
        return {
            ...
            'params': {
                ...
                'message': _('La nómina ha sido actualizada con los datos más recientes.'),
                'sticky': False,
            }
        }
